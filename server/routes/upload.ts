...
const processDocumentWithOCR = async (fileBuffer: Buffer, mimeType: string): Promise<ExtractedItem[]> => {
...
export const handleFileUpload: RequestHandler = async (req, res) => {
  try {
    ...
    const extractedItems = await processDocumentWithOCR(file.buffer, file.mimetype);
